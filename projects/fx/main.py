...
def formula1(self: Scene):
    ...
    restexts = [
        Tex("$a^2$").move_to(res2[0].get_center()), # NOSONAR
    ...
    realres2 = Tex("$+ab$").next_to(realres, RIGHT*0.5) # NOSONAR
    self.wait(1)
...
# (a+b)(a-b) = a^2 - b^2; a=3, b=1 NOSONAR
...
# (a+b)(a-b) = a^2 - b^2; a=3, b=1 NOSONAR
def formula3(self: Scene):
    res = Square(side_length=3).move_to(LEFT*4).set_fill(BLUE, opacity=0.7)
    text = Tex("$(x+a)(x+b)$").next_to(res, DOWN)
    self.play(Create(res), FadeIn(text))
    res2 = [
        Square(side_length=3, color=RED),
        Rectangle(height=3, width=1, color=BLUE),
