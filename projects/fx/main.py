...
def formula1(self: Scene):
    res = Square(side_length=3)
    text = Tex("$(a+b)^2$")
    res.set_fill(BLUE, opacity=0.7)
    res.move_to(LEFT*4)
    text.next_to(res, DOWN)
    ...
    texts[0].next_to(res2[3], DOWN)
    texts[1].next_to(res2[2], DOWN)
    restexts[0].move_to(res2[0].get_center())
    restexts[1].move_to(res2[1].get_center())
    restexts[2].move_to(res2[2].get_center())
    restexts[3].move_to(res2[3].get_center())
    ...
    realres = Tex("$a^2$")
    realres.next_to(texts[1], DOWN*2)
    self.play(Transform(groups[0], realres))
    realres2 = Tex("$+ab$")
    realres2.next_to(realres, RIGHT*0.5)
    self.wait(1)
    self.play(Transform(groups[1], realres2))
    realres3 = Tex("$+ab$")
    realres3.next_to(realres2, RIGHT*0.5)
    self.wait(1)
    self.play(Transform(groups[2], realres3))
    realres4 = Tex("$+b^2$")
    realres4.next_to(realres3, RIGHT*0.5)
    self.wait(1)
