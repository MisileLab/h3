...
async function loadSettings(): Promise<void> {
  ...
  getElement<HTMLInputElement>('font-size').value = String(defaultSettings.fontSize);
  getElement<HTMLInputElement>('background-opacity').value = String(defaultSettings.backgroundOpacity);
  getElement<HTMLSelectElement>('position').value = defaultSettings.position;
  getElement<HTMLSelectElement>('max-lines').value = String(defaultSettings.maxLines);
...
async function saveSettings(): Promise<void> {
  const settings: ExtensionSettings = {
    ...
    backgroundOpacity: parseInt(getElement<HTMLInputElement>('background-opacity').value),
    position: getElement<HTMLSelectElement>('position').value as 'top' | 'bottom',
...
function updateDisplayValues(): void {
  ...
  getElement('background-opacity-value').textContent = String(defaultSettings.backgroundOpacity);
}

function setStatus(type: string, message?: string): void {
  const statusEl = getElement('status');
  statusEl.className = `status ${type}`;
  statusEl.textContent = message || type.charAt(0).toUpperCase() + type.slice(1);
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  getElement('font-size').addEventListener('input', () => {
    defaultSettings.fontSize = parseInt(getElement<HTMLInputElement>('font-size').value);
    updateDisplayValues();
    applySettings();
  });

  getElement('background-opacity').addEventListener('input', () => {
    defaultSettings.backgroundOpacity = parseInt(getElement<HTMLInputElement>('background-opacity').value);
    updateDisplayValues();
    applySettings();
  });

  getElement('position').addEventListener('change', () => {
    defaultSettings.position = getElement<HTMLSelectElement>('position').value as 'top' | 'bottom';
    applySettings();
  });

  getElement('max-lines').addEventListener('change', () => {
    defaultSettings.maxLines = parseInt(getElement<HTMLSelectElement>('max-lines').value);
    applySettings();
  });
