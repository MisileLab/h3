...
class TextFormatter(BaseOutputFormatter):
  ...
  def format(self, result: ProcessingResult) -> str:
    ...
    for page in result.pages:
      ...
      for line in page.text_lines:
        text_line = line.text
        # Add address and coordinate information if available
        if hasattr(line, "nearest_address") and line.nearest_address:
          text_line += f" [Address: {line.nearest_address}, Coordinates: {line.x}, {line.y}]"
