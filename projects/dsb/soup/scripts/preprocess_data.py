...
def process_codesearchnet(tokenizer):
    ...
    def transform(example):
        ...
        # Clean up the comment: remove Javadoc-style tags and extra whitespace
    ...
    def filter_example(example):
        ...
        # and comments with no letters or numbers.
        ...
        # Filter based on code token length
...
def process_scicode(tokenizer):
    ...
    # Split the test set into validation and test (50/50)
    valid_test_split = dataset.train_test_split(test_size=0.5, seed=42)
    
    def transform(example):
        # SciCode problems are Python-based
        return {
            "text": f"Write a Python function to solve the following problem: {example['description']}",
            "target_code": example['solution']
        }
