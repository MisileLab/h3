...
class Query:
    ...
    def school(self, name: str) -> List[School]:
        ...
        a.raise_for_status()
        first_stage = [School(name=i["SCHUL_NM"], description="", callnumber=i["ORG_TELNO"], address=i["ORG_RDNMA"], homepage=i["HMPG_ADRES"], questions=[], school_type=i["HS_SC_NM"]) for i in a.json()["schoolInfo"][1]["row"]]
        for i in first_stage:
            data = db["info"].find_one({"name":name})
            if data is None:
                data = db["info"].insert_one({"name":name,"description":"","callnumber":i.callnumber,"address":i.address,"homepage":i.homepage,"questions":[],"comments":[],"school_type":i.school_type})
                continue
...
class Mutation:
    @strawberry.mutation
    def register(self, name: str, email: str, password: str, user_type: str) -> Account:
    ...
    def set_description(self, email: str, password: str, description: str) -> None:
        ...
        db["login"].update_one({"email":email,"password":password},{"$set":{"description":description}})

    @strawberry.mutation
    def add_review(self, target_email: str, email: str, password: str, description: str, stars: float) -> Review:
        ...
        data = {"name": user["name"], "description": description, "stars": stars}
        if data in target_user["reviews"]:
            target_user["reviews"].remove(data)
        target_user["reviews"].append(data)
        db["login"].update_one({"email":target_email},{"$set":{"reviews":target_user["reviews"]}})
        return Review(name=user["name"], description=description, stars=stars)

    @strawberry.mutation
    def add_question(self, target_name: str, email: str, password: str, description: str) -> Question:
        user = db["login"].find_one({"email":email,"password":password})
        school = db["info"].find_one({"name":target_name})
        if user is None:
            raise ValueError("Invalid email or password")
        if school is None:
            raise ValueError("Invalid school name (maybe you need to search school once)")
        data = {"name": user["name"], "description": description, "answers": []}
        school["questions"].append(data)
        db["info"].update_one({"name":target_name},{"$set":{"questions":school["questions"]}})
        return Question(name=user["name"], description=description, answers=[])

    @strawberry.mutation
    def add_answer(self, target_school: str, target_id: int, email: str, password: str, description: str) -> Answer:
        user = db["login"].find_one({"email":email,"password":password})
        ...
        if school is None:
            raise ValueError("Invalid school name (maybe you need to search school once)")
        data = {"name": user["name"], "description": description, "confirmed": False}
        school["questions"][target_id]["answers"].append(data)
        db["info"].update_one({"name":target_school},{"$set":{"questions":school["questions"]}})
        return Answer(name=user["name"], description=description, confirmed=False)

    @strawberry.mutation
    def confirm(self, email: str, password: str, target_school: str, target_id: int, confirm: bool) -> Answer:
        user = db["login"].find_one({"email":email,"password":password})
        school = db["info"].find_one({"name":target_school})
        if user is None:
            raise ValueError("Invalid email or password")
        ...
        db["info"].update_one({"name":target_school},{"$set":{"questions":school["questions"]}})
        return Answer(name=school["questions"][target_id]["answers"][0]["name"], description=school["questions"][target_id]["answers"][0]["description"], confirmed=confirm)
