...
class OpponentProfile:
  ...
  def __init__(self, name: str) -> None:
    ...
    # Opening repertoire
    self.white_openings = defaultdict(int)
    self.black_openings = defaultdict(int)

    # Move timing statistics
    self.avg_move_time = 0.0
    self.move_time_variance = 0.0
    self.time_pressure_errors = 0

    # Style metrics (0-1 scale)
    self.tactical_score = 0.5  # 0 = positional, 1 = tactical
    self.risk_score = 0.5  # 0 = conservative, 1 = aggressive
    self.complexity_preference = 0.5  # 0 = simple, 1 = complex

    # Phase performance
    self.opening_performance = 0.0
    self.middlegame_performance = 0.0
    self.endgame_performance = 0.0

    # Error patterns
    self.error_rate = 0.0
    self.blunder_rate = 0.0
    self.error_phase_distribution = {
      "opening": 0.0,
      "middlegame": 0.0,
      "endgame": 0.0
    }
...
class OpponentAnalyzer:
  ...
  def analyze_game(
    ...
    engine_eval: list[float] | None = None
  ) -> None:
