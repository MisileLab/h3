...
class EvaluationMetrics:
  ...
  def calculate_expert_usage(
    ...
    expert_usage: defaultdict[str, float] = defaultdict(float)
...
class EngineMatch:
  ...
  def play_match(self) -> dict[str, object]:
    """Play the match.
  ...
  def evaluate_position_batch(
    ...
    with torch.no_grad():
      for fen in positions:
        ...
        policy_logits, value, expert_weights = self.model(board_tensor, additional_features) # pyright: ignore[reportAny]
        ...
        move_probs.sort(key=lambda x: x[1], reverse=True) # pyright: ignore[reportAny]
        
        # Get the top moves
        top_moves = [move for move, _ in move_probs[:3]] # pyright: ignore[reportAny]
    ...
    metrics: dict[str, float] = {
      ...
      "top_3_accuracy": EvaluationMetrics.calculate_top_k_accuracy([top_moves for _, top_moves in zip(positions, predicted_moves)], target_moves, k=3), # pyright: ignore[reportArgumentType]
